...
**/
exports.sfcSfcTypeListPOST = function(args, res, next) {
    var val = proxy.getVal(args);
    var funcName = "sfcSfcTypeListPOST";

    logger.debug(funcName + ' | request:\n%s', JSON.stringify(val, null, '\t'));

    if (!proxy.run('services.SfcType.sfcSfcTypeListPOST', args, res)) {
        var mockup = require('../models/SfcTypeDTO');
