...
**/
exports.clusterSearchPOST = function(args, res, next) {
    var val = proxy.getVal(args);
    var funcName = "clusterSearchPOST";

    logger.debug(funcName + ' | request:\n%s', JSON.stringify(val, null, '\t'));

    if (!proxy.run('services.Cluster.clusterSearchPOST', args, res)) {
        var mockup = require('../models/ClusterDTO');
