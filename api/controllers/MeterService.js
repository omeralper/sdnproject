...
* request (MeterListRequest)
**/
exports.meterListPOST = function(args, res, next) {
    var val = proxy.getVal(args);
    var funcName = "meterListPOST";

    logger.debug(funcName + ' | request:\n%s', JSON.stringify(val, null, '\t'));

    if (!proxy.run('services.Meter.meterListPOST', args, res)) {
        var mockup = require('../models/MeterDTO');
