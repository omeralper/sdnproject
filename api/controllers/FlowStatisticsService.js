...
* request (UserFlowStatRequest)
**/
exports.flowStatsUserPOST = function(args, res, next) {
    var val = proxy.getVal(args);
    var funcName = "flowStatsUserPOST";

    logger.debug(funcName + ' | request:\n%s', JSON.stringify(val, null, '\t'));

    if (!proxy.run('services.FlowStatistics.flowStatsUserPOST', args, res)) {
