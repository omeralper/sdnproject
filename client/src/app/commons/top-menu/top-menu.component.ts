...
export class TopMenuComponent extends BaseComponent implements OnInit {
	...
	public domainChangeSub: Subscription;
    public hasPwdChangePerms:boolean;
    ...
    constructor(public eventsManager: EventsManager,
		...
		eventsManager.on<UserInfo>(AUTHENTICATION_EVENT_TYPE.USER_INFO_UPDATED, (event) => {
			if (is.existy(event.eventData) && is.existy(event.eventData.userDTO)) {
				...
				this.hasPwdChangePerms = this.baseServices.permManager.userHasPermissions("users:change_pwd") && !this.sessionManager.isLdapEnabled;
                this.updateRootMenu();
			} else {
				this.currentUser = "";
				this.rootMenu = null;
        ...
        // eventsManager.on(TOPOLOGYEVENTTYPE.DOMAIN_INFO_UPDATED, (event) => { //MLAT-4487
        //     this.getDomains();
        //     this.changeDetector.detectChanges();
        // });
