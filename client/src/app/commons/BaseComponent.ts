...
export abstract class BaseComponent implements OnInit, OnDestroy, OnChanges, AfterViewInit {
	...
	/**
	 * The array of all subscriptions the component has ever subscribed anywhere. This array of subscriptions will be unsubscribed
	 * at ngOnDestroy event of the component.
	 * @type {Array}
	 */
	public subscriptions:Array<ISubscription> = [];

    /**
	 * Has the status of current session mode. If we are in Central controller or not.
     */
    public isTopologyModeCentral: boolean;
	...
	ngOnInit() {
        ...
        this.isTopologyModeCentral = is.truthy(this.baseServices.sessionManager.getItem("isTopologyModeCentral"));
	}
