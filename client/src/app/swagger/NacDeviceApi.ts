...
import {DeviceQuarantineResponse} from "./DeviceQuarantineResponse";
import {DeviceQuarantineRequest} from "./DeviceQuarantineRequest";
...
export class NacDeviceApi  extends BaseApi {
    ...
    /**
     * Cihazın karantinaya alınmasını sağlayan fonksiyon.
     * 
     * @param request Genel karantina isteğinin belirtildiği veri yapısı.
     */
    public deviceQuarantinePOST (request: DeviceQuarantineRequest, isBlockUI:boolean=true, extraHttpRequestParams?: Headers ) : Observable<DeviceQuarantineResponse> {
        const path = this.basePath('/nas/device/quarantine');
        ...
        // verify required parameter 'request' is set
        if (!request) {
            throw new Error('Missing required parameter request when calling deviceQuarantinePOST');
        }else{
            var bodyParam = request;
            var body = JSON.stringify(bodyParam);
        }
        ...
        if (isBlockUI) this.block('deviceQuarantinePOST');
        return <Observable<DeviceQuarantineResponse>>this.http.post(path, body,{headers:headerParams})
            .map((response: Response) => response.json())
            .finally(()=>{ if (isBlockUI) this.unblock('deviceQuarantinePOST'); });
    }
