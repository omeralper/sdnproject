...
export class ControllerApi  extends BaseApi {
     ...
     * Kontrolcüyü kapatır.
     * 
     * @param request Kontrolcü numarasını içeren istek nesnesi.
     */
    public controllerHaltPOST (request: GenericIdRequest, isBlockUI:boolean=true, extraHttpRequestParams?: Headers ) : Observable<ControllerNodeResponse> {
        const path = this.basePath('/ctrl/controller/halt');
        ...
        // verify required parameter 'request' is set
        if (!request) {
            throw new Error('Missing required parameter request when calling controllerHaltPOST');
        }else{
            var bodyParam = request;
            var body = JSON.stringify(bodyParam);
        }
        ...
        if (isBlockUI) this.block('controllerHaltPOST');
        return <Observable<ControllerNodeResponse>>this.http.post(path, body,{headers:headerParams})
            .map((response: Response) => response.json())
            .finally(()=>{ if (isBlockUI) this.unblock('controllerHaltPOST'); });
    }
