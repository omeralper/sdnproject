...
export class DocumentConverter {
    ...
    addNewHost(data, allTopologyData) {
        ...
        let addNewHostProcess = (currentTopologyId)=> {
            if (this.isNodeUnique(newHostData, allTopologyData[currentTopologyId].data.nodes)) {
                ...
                let sw = PathCalculations.topologyFindSwitchDTOById(newHostData.port.id, allTopologyData[currentTopologyId].data);
                if (!this.isNotNullOrUndefinedOrEmptyString(sw)) {
    ...
    addNewLink(newLinkData, allTopologyData): any {
        ...
        var addLinkProccess = (currentTopologyId)=> {
            var needToUpdateBool = true; // if there's a reverse or duplicate link just update the object but return something so don't render
            ...
            if (this.isNotNullOrUndefined(dataSource) && this.isNotNullOrUndefined(dataTarget) && isLinkAlreadyExisting.length <= 0) { // if new link
                if (dataSource > -1 && dataTarget > -1) {
                    ...
                    console.log("source or target = -1");
                }
            } else if (this.isNotNullOrUndefined(dataSource) && this.isNotNullOrUndefined(dataTarget) && isLinkAlreadyExisting[0] > -1) { // if link exists (if its down)
                if (dataSource > -1 && dataTarget > -1) {
                    allTopologyData[currentTopologyId].data.links[isLinkAlreadyExisting[0]].isDown = false;
                } else {
                    console.log("source or target = -1");
                }
            } else {
        ...
        let updateLinksProcess = (currentTopologyId)=> {
            for (let i = 0; i < allTopologyData[currentTopologyId].data.links.length; i++) {
                ...
                if(currentLink.srcPort && currentLink.destPort){
                    if ( (currentLink.srcPort.id == newLinkData.srcPort.id && currentLink.destPort.id == newLinkData.destPort.id) || (currentLink.srcPort.id == newLinkData.destPort.id && currentLink.destPort.id == newLinkData.srcPort.id) ) {
                        currentLink.status = newLinkData.status;
                        if(newLinkData.status == LINKSTATUS.DOWN){
                            currentLink.isDown = true;
                        }else{
                            currentLink.isDown = false;
                        }
    ...
    updateStats(data, allTopologyData) {
        ...
        for (let j = 0; j < statsData.links.length; j++) {
            ...
            if (typeof allTopologyData[currentTopologyName].data.links[currentLink[0]] == "undefined") {
                ...
                allTopologyData[currentTopologyName].data.links[currentLink[0]] = this.portStatUpdate(allTopologyData[currentTopologyName].data.links[currentLink[0]], statsData.links[j]);
            }
