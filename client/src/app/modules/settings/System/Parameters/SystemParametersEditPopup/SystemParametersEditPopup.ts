...
export class SystemParametersEditPopup extends BasePopupEditPage<ModuleNodeConfigDTO> implements OnInit, AfterViewInit, OnChanges, OnDestroy {
	...
	save() {
		...
		if (this.isValueTypeNumber()) {
            try {
                this.data.configValue = parseFloat(this.data.configValue).toString();
            } catch (e) {
                this.data.configValue = "0";
            }

            if (this.data.configValue == 'NaN') this.data.configValue = "0";
        }
