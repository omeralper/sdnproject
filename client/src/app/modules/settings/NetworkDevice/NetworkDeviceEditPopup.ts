...
export class NetworkDeviceEditPopup extends BasePopupEditPage<NetworkDeviceDTO> {
                ...
                .subscribe(
                    (res) => {
                        if(this.baseServices.uiHelper.processResponse(res)){
                            ...
                            for(let i = 0; i < switchDTOList.length; i++){
                                this.switchListBox.append("<option>"+ switchDTOList[i].id +"</option>");
                            }
    ...
    save() {
        ...
        if(this.isNewItem){
            ...
            this.data.port.id = this.switchListBox.val();
