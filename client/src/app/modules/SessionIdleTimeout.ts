    ...
    init() {
        ...
        if ($document.idleTimer) {
            ...
            $document.on("idle.idleTimer", (event, elem, obj) => {
                ...
                var popup = this.pageServices.uiHelper.confirm($.t("common.messages.idle_timeout").replace("__seconds__", i + "")
                    , (selected: Action) => {
                ...
                let intId = setInterval(() => {
                    document.title = $.t("common.messages.idle_timeout_title").replace("__seconds__", i + "");
                    $(".bootbox-body", popup[0]).text($.t("common.messages.idle_timeout").replace("__seconds__", i + ""));
                    i--;
                    if (i < 0) {
                        document.title = t;
                        clearInterval(intId);
