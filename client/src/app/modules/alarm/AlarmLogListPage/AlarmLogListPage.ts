        ...
        if (this.$tableRef) {
            ...
            this.$dataTableRef.columns().every((idx) => {
                    ...
                    switch (column.type) {
                        case "num":
                            if (column.name == "severity") {
                                ...
                                let options = ['<option value="">' + this.t('labels.all') + '</option>'];
                                $.each(EnumHelper.getNames(ALARMSTATUS), (name, value)=> {
                                ...
                                let options = [];
                                $.each(EnumHelper.getNames(ALARMTYPE), (name, value)=> {
                ...
                $(".clear_input", $newRow).on("click", (e)=> {
                    $("#filter_" + $(e.currentTarget).data('index'), $newRow).val('');
                    this.updateSearch();
                });

                $("#filter_" + idx, $newRow).on("change", (e)=> {
                    //console.log($(e.currentTarget).val());
                    this.updateSearch();
                });

                // $("#filter_" + idx, $newRow).on("apply.daterangepicker", (ev, picker)=> {
