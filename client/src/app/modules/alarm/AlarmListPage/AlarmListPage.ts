    ...
    getDataTableOptions() {
        return {
            columns: [
                ...
                {title: this.ft('severity_short'),tooltip: this.ft('severity'), width: '4%', orderable: true, type: 'string', name: 'severity', className: "dt-center"},
                        ...
                        [
                            ...
                            this.renderRowActions(idx, true, (action: Action, idx: number, data: AlarmDTO) => {
                                if (action.id.lastIndexOf("emergency") != -1) {
                                    return data.resource === ALARMRESOURCE.EMERGENCY_POLICY;
                                }
                                return true;
                            })
                        ]
