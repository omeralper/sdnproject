...
import {NotPermittedAction, PermManager} from "app/modules/PermManager/PermManager";
...
export class MvtnEventProcessor extends EVENT_SYSTEM implements IEventProcessor {
    ...
    public notifyVTNRequest(eventData: AlarmDTO, dialogType: DIALOG_TYPES) {
        if (eventData.guiNotification == GUINOTIFICATION.VISIBLE
                && (this.isNetworkAdmin
                    || (is.existy(this.sessionManager.currentUser)
                        && is.existy(this.sessionManager.currentUser.userDTO)
                        && eventData.resourceId == this.sessionManager.currentUser.userDTO.username)
            )) {
    ...
    private get isNetworkAdmin():boolean {
        return this.permManager.checkPermission('phy_topo:manage:', NotPermittedAction.REMOVE) == NotPermittedAction.ALLOW;
    }
}
