...
export class ServiceQualityEditPopup extends BasePopupEditPage<ServiceActionDTO> implements OnInit, AfterViewInit, OnDestroy {
	...
	fetchSwitches() {
		...
		let s = this.topologyApi.topologyGetPOST(req).subscribe((res) => {
		...
		switch (this.currentRule) {
			...
			case 'ipHeaderRule':
				...
				if (this.data.changeHeaders.dstIp) {
					...
					delete this.data.changeHeaders.dstMac;
				}
	...
	save() {
		...
		if (this.typeName != null) {
			this.data.sfcType = this.typeList.filter((l) => {
				return this.typeName === l.sfcTypeName;
			})[0];
		}
