...
export class ServiceQualityPage extends BaseDataTablesPage<ServiceActionDTO> implements OnInit, OnDestroy, OnChanges, AfterViewInit {
    ...
    getDataTableOptions() {
        return {
            columns: [
                {title: '<input type="checkbox" class="group-checkable icheck">', type:'html', name:'id',visible:false},
                {title: this.ft('cat_name'), width: '15%', orderable: true, type: 'string', name: 'policyName'},
                {title: this.ft('class'), width: '15%', orderable: false, type: 'num', name: 'servicePolicyClass'},
                {title: this.ft('bandwidth'), width: '10%', orderable: true, type: 'num', name: 'bandwidth', className:'dt-center dt-right'},
                { width: '10%', orderable: false, type: 'string', name: 'maxBandwidthUnit',visible:false },
                {title: this.ft('priority'), width: '5%', orderable: true, type: 'num', name: 'priority', className:'dt-center dt-right'},
                {title: this.ft('mvtnId'), width: '8%', orderable: true, type: 'num', name: 'mvtnId'},
                {title: this.ft('required_devices'), width: '13.5%', orderable: true, type: 'num', name: 'includedDevices'},
                {title: this.ft('blocked_devices'), width: '13.5%', orderable: true, type: 'num', name: 'excludedDevices'},
                {title: this.ft('rules'), width: '7%', orderable: true, type: 'num', name: 'jitter', className: "dt-center dt-right"},
    ...
    handleRuleIcons(itm: ServiceActionDTO){
        ...
        //let empty =()=>result.push('-');
        let badge = (type)=>{
            result.push('<span class="badge badge-primary">',type,'</span>');
        };
        let empty =()=>{
        ...
        if(itm.reservedPath == RESERVEDPATH.ACTIVE){
            ...
            badge('<span style="text-decoration:line-through">R</span>');
        } else {
            empty();
        }
