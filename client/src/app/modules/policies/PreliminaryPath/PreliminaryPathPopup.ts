...
export class PreliminaryPathPopup extends BasePopupEditPage<ProactivePathPolicyDTO
	| any> implements OnInit, AfterViewInit, OnDestroy {
	...
	ngAfterViewInit() {
		if (super.ngAfterViewInit()) {
			...
			let s =
				this.mvtnApi.virtualListPOST(mvtnRequest).subscribe(
					(res) => {
		...
		$('#fromId').typeahead({
			...
			},
			{
				...
				source: (query, syncResults, asyncResults) => {
					...
					let s = this.topoApi.topologyGetPOST(req).subscribe((res) => {
						if (this.baseServices.uiHelper.processResponse(res)) {
