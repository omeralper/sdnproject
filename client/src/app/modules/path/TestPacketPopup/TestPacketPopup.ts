...
export class TestPacketPopup extends BasePopupPage<PathDTO> implements OnInit, AfterViewInit, OnDestroy {
	...
	startTest() {
		...
		$all_links.css("visibility", "hidden");
		...
		this.eventSubscription = this.eventsManager.on<any>(EVENT_SYSTEM.GENERIC_EVENT, (event) => {
			...
			if (event.eventData.detail && event.eventData.detail.includes(this.req.data.id)) {
				...
				try {
					var pack = JSON.parse(event.eventData.detail.substring(0, event.eventData.detail.indexOf("__")) + "}");
				}
				catch (e) {
					$all_links.css("visibility", "visible");
				}

				this.packCount++;

				let pack_delay = Number(pack.delay);

				if (pack.type === "test") {
					this.testResult += this.t('labels.pack') + "#" + this.packCount + ": [" + this.t('labels.pack_delay') + ": " + pack_delay.toFixed(2) + "ms]\n";
				}
				else {
					this.testResult += "Test#" + this.testCount + ":" + " [" + this.t('labels.test_pack') + ": " + pack.packet_count + ", " + this.t('labels.test_delay') + ": " + pack_delay.toFixed(2) + "ms]\n\n";
