...
export class UIHelper {
    ...
    //constructor(@Inject('ISessionInvalidator') public sessionInvalidator: ISessionInvalidator) { //public logger: Logger) { //, public router: Router
    ...
    public populateTooltips($container?: JQuery): JQuery {
        ...
        if ($cnt) {
            ...
            setTimeout(() => $('[rel=tooltip],[data-toggle=tooltip]', $cnt).tooltip(), 300);
        }
        return $cnt;
    }
    ...
    /*    public initICheckPlugin(container?:JQuery) {
     ...
     var icheckElements = container ? $(".icheck-me", container) : $(".icheck-me");

     if (icheckElements.length > 0) {
     icheckElements.each(function () {
     var $el = $(this);
    ...
    processResponse(data: (string | GenericResponse | ProgressEvent | Response), successMessage: (
        ...
        if (is.existy(data)) {
            ...
            if (is.existy(res.status)) { //} && is.existy(res.token)) {
                ...
                switch (res.status) {
                    ...
                    case RETURNSTATUS.INVALID_SESSION: {
                        let sessionInvalidator = <ISessionInvalidator>this.injector.get('ISessionInvalidator');
...
export interface DialogCallbackType { (selected: Action): void
}
