...
export class CreateVirtualTopology extends BasePopupEditPage<TopologyInfoDTO> implements OnInit, AfterViewInit, OnDestroy {
				...
				this.topologyApi
					...
					.subscribe(
						(res) => {
							...
							this.currentName = res.data && res.data.info ? res.data.info.name : "";
							this.currentVlan = res.data && res.data.info ? res.data.info.vlanTag : 0;
						}
					);
			}
		}
				...
				.subscribe(
					(res) => {
						...
						if (this.baseServices.uiHelper.processResponse(res)) {
							...
							this.data.energyConsumption = this.result.energyConsumption;
	...
	save() {
		...
		if (!form.checkValidity()) {
			//mark tabs with validation errors
			$(".validation-error", this.$container).parents('.tab-pane').each((idx, itm) => {
				$("a[href='#" + $(itm).attr('id') + "']", this.$container).addClass('label-required');
			});

			this.baseServices.uiHelper.notify($.t('common.messages.validation_error'), DIALOG_TYPES.WARNING);
			return;
		} else {
			//remove marks from tabs with validation errors
			$("a[data-toggle='tab']", this.$container).removeClass('label-required');
		}
