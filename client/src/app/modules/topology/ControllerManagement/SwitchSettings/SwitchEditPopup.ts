    ...
    ngAfterViewInit() {
        if (super.ngAfterViewInit()) {
            ...
            $(".switch-meter-mode", this.$container)
                .bootstrapSwitch('state', this.data.isMeterEnabled == true)
                .on('switchChange.bootstrapSwitch', (event, state) =>
                    { this.data.isMeterEnabled = !!state; this.changeDetector.detectChanges(); }
                );

            $(".is-controller-device", this.$container)
                .bootstrapSwitch('state', this.data.isControllerDevice == true)
                .on('switchChange.bootstrapSwitch', (event, state) =>
                    { this.data.isControllerDevice = !!state; this.changeDetector.detectChanges(); }
                );

            $(".power-saver-mode", this.$container)
                .bootstrapSwitch('state', this.data.powerSaverModeEnabled == true)
                .on('switchChange.bootstrapSwitch', (event, state) => {
