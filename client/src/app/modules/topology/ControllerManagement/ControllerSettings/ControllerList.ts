...
export class ControllerList extends BaseDataTablesPage<ControllerNodeDTO> implements OnInit, OnDestroy, OnChanges, AfterViewInit {
    ...
    getDataTableOptions() {
        return {
            columns: [
                {title: this.ft('status'), width: '15%', orderable: true, type: 'string', name: 'status'},
                {
                ...
                {title: this.ft('ip'), width: '15%', orderable: true, type: 'string', name: 'ip'},
                {title: this.ft('port'), width: '5%', orderable: true, type: 'string', name: 'port'},
