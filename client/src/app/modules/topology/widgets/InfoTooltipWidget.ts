...
export class InfoTooltipWidget extends BasePage implements OnInit, OnDestroy, OnChanges, AfterViewInit {
    ...
    constructor(public changeDetection: ChangeDetectorRef, public elementRef: ElementRef, baseServices: PageServices, public documentConverter: DocumentConverter, public sharedService: SharedService) {
    ...
    public renderHostTable(hostDTO: HostDTO): IInfoData {
        ...
        if (is.existy(hostDTO.port)) {
            if (is.existy(hostDTO.port.address)) {
                ...
                if (is.existy(hostDTO.port.address.mac)) infoPanel.entityRow("mac", hostDTO.port.address.mac);
            }
        }
        ...
        if (is.not.empty(hostDTO.networks)) {
            ...
            infoPanel.entityRow("vtn", hostDTO.networks.join(','));
        }
...
class InfoPanelBuilder {
        ...
        let genStat = (stat) => {
            switch (name) {
                ...
                case "labels.stat_fields.errors":
                    return is.existy(stat.packetErrors) ? {
                ...
                case "labels.stat_fields.drops":
                    return is.existy(stat.packetDrops) ? {
    ...
    private statRow2(name: string, source: any, target?: any): IStatInfo {
        let nameData = this.parent.t(name, {returnObjectTrees: true});
        ...
        if (is.existy(source) && is.existy(source.srcPort)) {
            ...
            if (is.existy(target) && is.existy(target.srcPort)) {
                statsInfo = [
                    ...
                    this.statRow2("labels.stat_fields.delay", source.delay, target.delay),
                    this.statRow2("labels.stat_fields.jitter", source.jitter, target.jitter)
                ]
            } else {
                statsInfo = [
                    ...
                    this.statRow2("labels.stat_fields.loss", source.loss),
                    this.statRow2("labels.stat_fields.delay", source.delay),
                    this.statRow2("labels.stat_fields.jitter", source.jitter)
                ]
            }
        }
