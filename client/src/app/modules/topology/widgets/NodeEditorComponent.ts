...
export class NodeEditorComponent extends BasePage implements OnInit, OnDestroy, OnChanges, AfterViewInit, OnChanges {
    ...
    @Input() topologyCreateModeFlag: boolean;
        ...
        this.mvtnApi.virtualListDeviceProfilesPOST(deviceProfileRequest).toPromise().then(
            (res:DeviceProfileListResponse) => {
                if (this.baseServices.uiHelper.processResponse(res)) {
                    ...
                    if(this.selectedElementToEdit.deviceProfile && this.selectedElementToEdit.deviceProfile.profileId && this.topologyData.status === TOPOLOGYSTATUS.ACTIVE ){
                        for(let i = 0; i < this.optionList.length; ++i){
                            if(this.optionList[i].profileId == this.selectedElementToEdit.deviceProfile.profileId){
                                for(let p = 0; p < this.selectedElementToEdit.mvtnPortInfo.portDetails.length; ++p){
                                    this.optionList[i].mvtnPortInfo.portDetails.push(this.selectedElementToEdit.mvtnPortInfo.portDetails[p]);
                                }
                                this.optionList[i].mvtnPortInfo.portDetails.sort((a, b)=> Number(a.number) - Number(b.number));
                            }
                        }
                    }
    ...
    updateSelectedValue(e, a){//html binding
        for(let i = 0; i < this.optionList.length; i++){
            if(this.optionList[i].profileName || this.optionList[i].profileId){
                if( (this.optionList[i].profileName && (this.optionList[i].profileName ==  a.deviceProfile.profileName)) || ( this.optionList[i].profileId && (this.optionList[i].profileId == a.deviceProfile.profileId)) ){
        ...
        if(nodes.length > 0){
            for( let i = 0; i < list.length; i++ ){
                for(let j = 0; j < nodes.length; j++){
                    if( nodes[j].deviceProfile && (list[i].profileId == nodes[j].deviceProfile.profileId) && list[i]['isUsed'] != true){
                        list[i]['isUsed'] = true;
                    }else{
