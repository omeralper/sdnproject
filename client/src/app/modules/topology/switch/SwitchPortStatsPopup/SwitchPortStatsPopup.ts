    ...
    handleConfigRuleIcons(itm: Array<PORTCONFIG>){
        ...
        if(Object.keys(itm).length > 0){
            if(itm.indexOf(PORTCONFIG.PORT_DOWN) > -1){
                result.push('<span class="badge badge-danger" title="' + this.t("messages.port_down") + '">P</span>');
            }else{
                empty();
            }

            if(itm.indexOf(PORTCONFIG.NO_FWD) > -1){
                result.push('<span class="badge badge-primary" style="text-decoration:line-through" title="' + this.t("messages.no_fwd") + '">F</span>');
            }else{
                empty();
            }

            if(itm.indexOf(PORTCONFIG.NO_PACKET_IN) > -1){
                result.push('<span class="badge badge-primary" style="text-decoration:line-through" title="' + this.t("messages.no_packet_in") + '">PI</span>');
            }else{
                empty();
            }

            if(itm.indexOf(PORTCONFIG.NO_RECV) > -1){
                result.push('<span class="badge badge-primary" style="text-decoration:line-through" title="' + this.t("messages.no_recv") + '">R</span>');
            }else{
                empty();
            }
        } else {
    ...
    handleStateRuleIcons(itm: Array<PORTSTATE>){
        ...
        if(Object.keys(itm).length > 0){
            if(itm.indexOf(PORTSTATE.BLOCKED) > -1){
                result.push('<span class="badge badge-primary" title="' + this.t("messages.blocked") + '">B</span>');
            }else{
                empty();
            }

            if(itm.indexOf(PORTSTATE.LINK_DOWN) > -1){
                result.push('<span class="badge badge-danger" title="' + this.t("messages.link_down") + '">D</span>');
            }else{
                empty();
            }

            if(itm.indexOf(PORTSTATE.LIVE) > -1){
                result.push('<span class="badge badge-success" title="' + this.t("messages.live") + '">L</span>');
            }else{
                empty();
            }

            if(itm.indexOf(PORTSTATE.STP_LISTEN) > -1){
                result.push('<span class="badge badge-info" title="' + this.t("messages.stp_active") + '">S</span>');
            }else{
                empty();
            }
        } else {
