 ...
 */
import { Component, OnInit, OnDestroy, OnChanges, AfterViewInit, ElementRef, Input, Output, EventEmitter, ViewChild, ChangeDetectorRef, ApplicationRef } from "@angular/core";
...
import {DEVICEEDITPOPUPMODE, SwitchEditPopup2} from "../ControllerManagement/SwitchSettings/SwitchEditPopup";
...
export class ShowTopology extends BasePage implements OnInit, OnDestroy, OnChanges, AfterViewInit {
    ...
    emitTopologyEditPopup(){this.emitToTopologyEditPopup.emit();}
    ...
    get initialZoomTranslate() { if(this.topologyData.tabType == CREATESHOWENUM.CREATE){return [0,0]}else{return ShowTopology._initialZoomTranslate[(this.topologyData && this.topologyData.id)?this.topologyData.id:'__default'] || ShowTopology._initialZoomTranslate['__default']; }}
    set initialZoomTranslate(value) { ShowTopology._initialZoomTranslate[(this.topologyData && this.topologyData.id)?this.topologyData.id:'__default'] = value; }
    get initialZoomScale() { return ShowTopology._initialZoomScale[(this.topologyData && this.topologyData.id)?this.topologyData.id:'__default'] || ShowTopology._initialZoomScale['__default']; }
    set initialZoomScale(value) { ShowTopology._initialZoomScale[(this.topologyData && this.topologyData.id)?this.topologyData.id:'__default'] = value; }
    ...
    topoInit() { //Renders the topology with current selected data so in order for this to work properly, set the topologyData field beforehand and then call this method
        ...
        if(this.topologyData.data.nodes.length >= 100){
            this.clusterIt(this.topologyData, this.clusterTopologyData);
        }else{
    ...
    setJQueryFields() {
        ...
        //this.navHeight = $('.navbar').innerHeight();

        //this.footerFlag = this.stringBooleanReverser(this.footerFlag);
        //this.detailInfoFlag = this.stringBooleanReverser(this.detailInfoFlag);
        //this.toggleFooter();
        //this.toggleDetailInfo();
        this.toggleStringAndIcons(this.footerFlag, this.toggleFooterInfoBtn, 'network_vis.topology.show_footer', 'network_vis.topology.hide_footer', this.footerBtnIcon, 'icon-empty', 'icon-eye');
        this.toggleStringAndIcons(this.detailInfoFlag, this.toggleDetailInfoBtn, 'network_vis.topology.show_detail_info', 'network_vis.topology.hide_detail_info', this.detailBtnIcon, 'icon-empty', 'icon-eye');
        this.toggleStringAndIcons(this.controllersTabFlag, this.toggleControllersTabBtn, 'network_vis.topology.show_controllers_tab', 'network_vis.topology.hide_controllers_tab', this.controllerTabIcon, 'icon-empty', 'icon-eye');
        this.toggleStringAndIcons(this.portShowHideFlag, this.togglePortsBtn, 'network_vis.topology.show_ports', 'network_vis.topology.hide_ports', this.togglePortsIcon, 'icon-empty', 'icon-eye');
        this.toggleStringAndIcons(this.trMapFlag, this.toggleTrMapFlagBtn, 'network_vis.topology.show_trmap', 'network_vis.topology.hide_trmap', this.trMapIcon, 'icon-empty', 'icon-eye');
    ...
    updateLinkStats(eventData) {
        let links = this.svg.selectAll('g.link').filter((link) => {
            if(eventData && eventData.links && eventData.links.length > 0){
                for (let i = 0; i < eventData.links.length; i++) {
                    ...
                    if (this.documentConverter.isNotNullOrUndefinedOrEmptyString(updatedLink.srcPort) && this.documentConverter.isNotNullOrUndefinedOrEmptyString(link.srcPort)) {//if switch link
                        if (updatedLink.srcPort.id == link.srcPort.id) {
                            link.srcPort.stats = updatedLink.srcPort.stats;
                            link.destPort.stats = updatedLink.destPort.stats;
                            link.status = updatedLink.status;
                            return link;
                        } else if (updatedLink.srcPort.id == link.destPort.id) {
                            link.destPort.stats = updatedLink.srcPort.stats;
                            link.srcPort.stats = updatedLink.destPort.stats;
                            link.status = updatedLink.status;
                            return link;
                        }
                    }
    ...
    render(data, renderingType: renderType, eventType = "ADD_REMOVE") {
        ...
        sourceText.insert("textPath")
            .attr("startOffset", (d) => this.d3attributes.textStartOffSet(d, "20%", "80%", "41%"))
            .attr("xlink:href", (d) => this.textLinkGenerator(d, true))
            .text((d) => {
                if ( this.portCondition(d) ) {
                    if (d.destPort.number && this.portShowHideFlag === "true") {
                        if (this.documentConverter.isNotNullOrUndefined(d["reverse"])) {
                            return "⇒";
                        }
                    }
                }
        ...
        targetText.insert("textPath")
            .attr("startOffset", (d) => this.d3attributes.textStartOffSet(d, "80%", "20%", "8%"))
            .attr("xlink:href", (d) => this.textLinkGenerator(d, true))
            .text((d) => {
                if ( this.portCondition(d) ) {
                    if (d.srcPort.number && this.portShowHideFlag === "true") {
        ...
        if(this.clusterTopologyData && this.clusterTopologyData.nodes){
        ...
        let nodeEnter = this.svgNode.enter().append("g")
                    ...
                    if (that.pathSelectionFlag && !that.topologyCreateModeFlag) {
                        if (!d3.select(this).classed("selected") && $('.pathSource').length === 0) {//choose source
                            ...
                            if(that.pathSelectionType === pathType.PROACTIVE){ //if the nodes were down to 0.2 opacity in the above code
                                that.svg.selectAll("[nodetype='hostnode'").filter((node) => {return node}).select("circle").transition().style("opacity", 1);
                            }
                            d.colorCode = "";
                            ...
                            if($('.pathTarget').length >= 1){// if a target is already selected and we want to select another target on the fly to make a new request, de-select the previous target
                                d3.selectAll(".pathTarget").classed({"selected": false, "pathTarget": false})
                                    .filter((node) => {node.colorCode = null; return node;}).select("circle")
            ...
            .on("dblclick", (d)=> {
                if(d.type === "cluster"){
                    that.expand[d.group] = !that.expand[d.group];
                    that.clusterTopologyData = that.topologyClusterService.network(that.topologyData.data, that.clusterTopologyData, that.topologyClusterService.getGroup, that.expand);

                    return that.render(that.clusterTopologyData, renderType.UPDATE);
                }else if(d.type === SUPERTOPOLOGYTYPE.DOMAIN){ //MLAT-3911
                    that.sharedService.annonceDomainChange(d);
                }
            });
    ...
    tick() {
        ...
        this.svgLink.selectAll("path.path-link")
            .attr("d", function (d) {
                if (!d.blocked && !d.isMvtnLinkUsed && d.selfLink !== true && d.isDown !== true) { //normal link
                    ...
                    let linkVector = new Vector2(d.target.x - d.source.x, d.target.y - d.source.y).getUnitVector();
                    let perpVector = linkVector.perpendicularClockwise();
                    let srcVector = perpVector.scale(sourceDelta);
                    let dstVector = perpVector.scale(targetDelta);
                    ...
                    if ((that.documentConverter.isHost(d.target.type) || that.documentConverter.isHost(d.source.type)) || (that.documentConverter.isBgpRouter(d.target.type) || that.documentConverter.isBgpRouter(d.source.type))) {
                        ...
                        if(d.linkOrder > 0){
                            ...
                            return "M" + (d.source.x + srcVector.X) + "," + (d.source.y + srcVector.Y) +
                                " A " + dr + "," + dr + " 0,0,1 " + (d.target.x + dstVector.X ) + "," + (d.target.y + dstVector.Y) +
                                " L" + (d.target.x - dstVector.X) + "," + (d.target.y - dstVector.Y) +
                                " A " + dr + "," + dr + " 0,0,0 " + (d.source.x - srcVector.X) + ", " + (d.source.y - srcVector.Y) +
                                " Z";
                        }else{
                            return "M" + (d.source.x + srcVector.X) + "," + (d.source.y + srcVector.Y) +
                                " L" + (d.target.x + dstVector.X ) + "," + (d.target.y + dstVector.Y) +
                                " L" + (d.target.x - dstVector.X) + "," + (d.target.y - dstVector.Y) +
                                " L" + (d.source.x - srcVector.X) + ", " + (d.source.y - srcVector.Y) +
                                " Z";
        ...
        this.svgLink.selectAll("text.port-source > textPath.text-path")
            .attr("startOffset", function (d) {
                if (!d.blocked && !d.isMvtnLinkUsed && d.selfLink !== true && d.isDown !== true) { //normal link
                    let angle = new Vector2(d.target.x - d.source.x, d.target.y - d.source.y).angle();
        ...
        this.svgLink.selectAll("lineargradient")
            .filter(function (d) {
                let linkVector = new Vector2(d.target.x - d.source.x, d.target.y - d.source.y).getUnitVector();
                let gradientVector = linkVector.scale(0.5);
        ...
        if (this.topologyData.type === TOPOLOGYTYPE.PHYSICAL) {
            ...
            if ((d as SwitchDTO).deviceInfo && this.documentConverter.isSwitch(d.deviceInfo.type) && d.deviceInfo.type !== DEVICETYPE.IP_PHONE && !this.topologyCreateModeFlag) {
                ...
                d3.select('#edit-switch').on("click", () => {
                    this.sharedService.showModal(SwitchEditPopup2, {switchData: d, mode: DEVICEEDITPOPUPMODE.TOPOLOGY}, (result) => {
                        if (result.isSuccess) {
                            d.name = result.data.name;
                            d.securityLevel = result.data.securityLevel;
                    ...
                    .html((i, v) => {
                        ...
                        if (d.reverse && this.topologyData.tabType == CREATESHOWENUM.SHOW) {
                            contextMenu.push("<li class='contextmenu-item' id='edit-downlink'> <a><i class='fa fa-gear'></i>" + this.i18n.t('links.edit.actions.edit_downlink.title') + "</a> </li>"+
                                ...
                                "<li class='contextmenu-item' id='stop-delay'> <a><i class='fa fa-stop'></i>" + this.i18n.t('links.edit.actions.stop_delay.title') + "</a> </li> </ul> </li>");
                        }
    ...
    toggleHosts() {
        ...
        this.toggleStringAndIcons(this.showHostsFlag, this.toggleHostsBtn, 'network_vis.topology.show_hosts', 'network_vis.topology.hide_hosts', this.hostsToggleIcon, 'icon-empty', 'icon-eye');
    }
    ...
    toggleVirtualGateways(){
        ...
        this.toggleStringAndIcons(this.showVGatewaysFlag, this.toggleVgatewaysBtn, 'network_vis.topology.show_vgateway', 'network_vis.topology.hide_vgateway', this.vGatewayToggleIcon, 'icon-empty', 'icon-eye');
    }
    ...
    resetPath(hideProactiveAlternavitesButton: boolean = true) {
        ...
        if (this.svg && this.svg.selectAll('.link').length > 0) {
            ...
            $("[r=" + topoConfig.hostClickedRadius + "]").attr("r", topoConfig.hostRadius).css({"stroke": topoConfig.defaultHostPeripheryColor,"fill": topoConfig.defaultHostFillColor});
    ...
    drawPath(pathData?: ProactivePathPolicyResponse) {
        ...
        let request = this.baseServices.apiHelper.genRequest(<PathRequest>{
            data: <PathDTO> {
                ...
                topologyId: (this.topologyData.type === TOPOLOGYTYPE.VIRTUAL) ? this.topologyData.name : '',
                appId: 'tr.com.argela.prognet.rest.resources.PathResource',
    ...
    addLinkButton() {//html
        if (d3.selectAll(".node").data().length > 1) {
            ...
            let link: LinkDTO = this.baseServices.apiHelper.genDTO({
                ...
                constraints: {availableBandwidth: this.topologyData.bandwidth, securityLevel: this.topologyData.linkSecurityLevel},
        ...
        if (!this.topologyData.isMappedVersionShown) {
            ...
            let data = <MvtnRequest>this.baseServices.apiHelper.genRequest({
                data: <MvtnDTO> this.baseServices.apiHelper.genDTO({
                    id:  (this.topologyData.tabType != CREATESHOWENUM.EDIT) ? null : this.topologyData.id,
            ...
            let mvtnReqData = <MvtnCreateRequest>this.baseServices.apiHelper.genRequest({
                data: <MvtnRequestDTO> this.baseServices.apiHelper.genDTO({
                    id: (this.topologyData.tabType == CREATESHOWENUM.EDIT && this.topologyData.type === TOPOLOGYTYPE.VIRTUAL_REQUEST) ? this.topologyData.id : ( (this.topologyData && this.documentConverter.isNotNullOrUndefined(this.topologyData.id) && this.topologyData.id.substring(0, 2) != "UI" && this.topologyData.type !== TOPOLOGYTYPE.VIRTUAL) ? this.topologyData.id : null ),
                    mvtnId: (this.topologyData.tabType != CREATESHOWENUM.EDIT) ? null : this.topologyData.id,
    ...
    clusterIt(topologyData, clusterTopologyData){
        this.render(this.topologyClusterService.clusterIt(topologyData, clusterTopologyData), renderType.FULL); //should be update for button
    }
