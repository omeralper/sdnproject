...
import {ReactivePathListReturn, REACTIVE_PATH_LIST_COMMAND, PathListPopup} from "../../path/PathListPopup/PathListPopup";
...
import {ProactivePathListPopup, ProactivePathListReturn,PROACTIVE_PATH_LIST_COMMAND} from "../../policies/PreliminaryPath/ProactivePathListPopup";
...
export class Topology extends BasePage implements OnInit, OnDestroy, OnChanges, AfterViewInit {
    ...
    onTopologyTrigger(transferData){//html
        ...
        if(transferData.mode != CREATESHOWENUM.EDIT){//create, show, none
            ...
            delete Topology.allTopologyData[transferData.tempId];
            ...
            if (result.isSuccess){
                for(let i=0; i<this.tabList.length; i++){
                    if(result.data.id === this.tabList[i].id){
                        ...
                        this.tabList[i].name = this.selectedTopologyData.name;
            ...
            if(Topology.allTopologyData.hasOwnProperty(topologyNameOfEvent) || event.revision == 999333){
                ...
                switch (data.type) {
                    ...
                    case TOPOLOGYEVENTTYPE.LINK_UPDATED:
                        ...
                        //if (Topology.currentTabId == data.topologyId || (data.id == "" && Topology.allTopologyData[Topology.currentTabId].type == TOPOLOGYTYPE.PHYSICAL)) {
    ...
    flattenSubTopology(res: SubTopologyResponse, shouldSelectDefaultSelected = true, id?) {
        ...
        if(shouldSelectDefaultSelected){
            selectedSubTopologyData = res.data.subTopologyList.find((v)=>{return v.id == res.data.selectedSubTopologyId;});
        }else{
            selectedSubTopologyData = res.data.subTopologyList.find((v)=>{return v.id == id;});
        }

        let mdata = this.topologyService.convert(selectedSubTopologyData);

        for (let i = 0; i < selectedSubTopologyData.subLinks.length; i++) {
            let currentVLink = selectedSubTopologyData.subLinks[i];

            currentVLink['source'] = this.documentConverter.getNodeById(currentVLink.srcPort.id, mdata.nodes);
            currentVLink['target'] = this.documentConverter.getNodeById(currentVLink.destPort.id, mdata.nodes);

            //currentVLink.source = currentVLink.srcPort;
            //currentVLink.target = currentVLink.destPort;

            currentVLink.srcPort = currentVLink['source'];
            currentVLink.destPort = currentVLink['target'];

            currentVLink['isMvtnLink'] = true;
            currentVLink['isMvtnLinkUsed'] = true;

            currentVLink = new tplLink(currentVLink);
            mdata.links.push(currentVLink);
        }
        ...
        for (let j = 0; j < selectedSubTopologyData.subLinks.length; j++) {
            for (let k = 0; k < selectedSubTopologyData.subLinks[j].physicalLinks.length; k++) {
                let currenVPMapLink = selectedSubTopologyData.subLinks[j].physicalLinks[k];
                let linkIndex = this.documentConverter.findLinkIndexByNodeIds(mdata.links, [currenVPMapLink.srcPort.id, currenVPMapLink.destPort.id], mdata.nodes)[0];
                if(this.documentConverter.isNotNullOrUndefinedOrEmptyString(linkIndex) && this.documentConverter.isNotNullOrUndefinedOrEmptyString(mdata.links[linkIndex])){
                    mdata.links[linkIndex].opacity = 1;
                    if(j == 1){
                        mdata.links[linkIndex].colorCode = topoConfig.virtualSublinkAlternativePathFromColor;
                        if(mdata.links[linkIndex]["reverse"])mdata.links[linkIndex]["reverse"].colorCode = topoConfig.virtualSublinkAlternativePathFromColor;
                    }
                }else{
                    console.log("ERROR on link index: " + linkIndex);
                }
            }
        }
    ...
    showTopology(topologyInfo: TopologyInfoDTO) {
        let tabData: TopologyTabData = {id:topologyInfo.id, topoType: topologyInfo.type, tabRank: ++Topology.tabRank, tabType: CREATESHOWENUM.NONE}; //dummy, only need id to check if tab is unique
        setTimeout(()=> {
    ...
    toggleBandwidthUtilizationSensitivity(){//html
        ...
        this.showTopologyComponent.toggleStringAndIcons(this.bwSensitivityFlag, this.bwSensitivityBtn, 'network_vis.topology.show_link_sensitivity', 'network_vis.topology.hide_link_sensitivity', this.bwSensitivityIcon, 'icon-empty', 'icon-eye');
    }
