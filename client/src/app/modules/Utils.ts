...
export class Utils {
    ...
    sortedObjectKeysByValue(obj) {
        if (is.existy(obj)) {
            let keys = Object.keys(obj);
            keys.sort((a,b) => {
                return obj[a].toString().localeCompare(obj[b].toString());
            });
            return keys;
        }
        return [];
    ...
    private delayedExecMap = {};
    delayedExec(key: string, timeout: number, callback: () => void) {
        let timeoutId = this.delayedExecMap[key];
        clearTimeout(timeoutId);
        this.delayedExecMap[key] = setTimeout(()=>{
            callback();
            delete this.delayedExecMap[key];
        },timeout);
    }
}
